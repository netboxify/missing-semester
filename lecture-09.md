...
Koliko bitova entropije je dovoljno? Zavisi od vašeg modela prijetnje. Za online nagađanjem, kao što XKCD comic iznosi, ~40 bitova entropije je prilično. Da bi bili optorni na offline nagađanje, jača lozinka će biti neophodna (npr. 80 bitova, ili više).
...
- Otporna na sudar - teško je pronaći dva inputa `m_1` i `m_2` tako da je `hash(m_1) = hash(m_2)` (imajte u vidu da je ovo mnogo jače svojstvo u odnosu na ciljanog otpora na udar)
...
## Simtrična kriptografija

Skrivanje sadržaja poruke je vjerovatno prvi koncept koji vam padne na pamet kada razmišljate o kriptografiji. Simtrična kriptografija ovo izvršava sa sledećim setom funkcionalnosti:
...
Encrypt/decrypt funkcije imaju svojstva koja su slična njihovim analozima iz simetričnih kriptosistema. Poruka može biti enkriptovana koristeće _public_ ključ. Uzimajući u obzir output (ciphertext), teško je odrediti input (plaintext) bez _privatnog_ ključa. Decrypt funkcija ima očigledna svojstva ispravljanja, tako da je `decrypt(encrypt(m, public key), private key) = m`.
...
Enkripcija čitavog diska vašeg laptop-a je lak način da zaštitite vaše podatke u slučaju da je vaš laptop ukraden. Možete koristiti [cryptsetup + LUKS](https://wiki.archlinux.org/index.php/Dm-crypt/Encrypting_a_non-root_file_system) na Linuxu, [BitLocker](https://fossbytes.com/enable-full-disk-encryption-windows-10/) na Windowsu, ili [FireVault] na macOS. Ovo će enkriptovati čitav disk sa simetričnim cipher-om, i sa ključim koji je zaštićen frazom.
